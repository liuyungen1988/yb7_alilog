...
func formatAddTraceId(format string) string{
  traceId := cls_skywalking_client_go.GetGlobalTraceId()
  if len(traceId) != 0 {
    format = "TraceId:" + traceId + " " + format
  }
  return format
}
...
func (l *SLog) doLog(level string, format string, v ...interface{}) {
  ...
  format = formatAddTraceId(format)
